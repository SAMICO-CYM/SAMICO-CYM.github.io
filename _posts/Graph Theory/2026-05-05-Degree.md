...
---
## Corollary
Let $D = (d_1, d_2, \cdots, d_n)$ be a nondecreasing sequence. Then $D$ is graphic $\iff$ the sequence $D' = (d_1, d_2, \cdots, d _ {n-d_n-1}, d _ {n - d_n}-1, \cdots, d_{n-1} - 1, 0)$ is graphic.

### Proof
If $D$ is graphic, then there is a realisation $G$ of $D$ such that the vertex $v_n$, with $\mathrm{deg}_G (v_n) = d_n$, is adjacent to $v _ {n-1}, \cdots, v _ {n - d_n}$ by Havel-Hakimi Theorem. By removing the vertex $v_n$ from $G$, we get a realisation $G'$ of $D'$. Thus $D'$ is graphic.
