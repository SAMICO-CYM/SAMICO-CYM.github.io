...
$$\langle p \rangle = m \langle v \rangle = m \frac{d \langle x \rangle}{dx}$$

$$\begin{align*} 
\frac{d \langle x \rangle}{dx} &= \frac{d}{dt} \int \Psi^\ast x \Psi \, dx \\
&= \int \frac{\partial}{\partial t} (\Psi^\ast x \Psi) \, dx \\
&= \int x(\Psi^\ast_t \Psi + \Psi^\ast \Psi_t) \, dx \\
&= \int x(\Psi (- \frac{i \hbar}{2m} \Psi_{xx} + \frac{i}{\hbar} V \Psi) + \Psi^\ast (\frac{i \hbar}{2m} \Psi_{xx} - \frac{i}{\hbar} V \Psi)) \, dx \\
...
&= \frac{i \hbar}{2m} \left[ x(\Psi^\ast \Psi_x - \Psi \Psi^\ast_x)_{-\infty}^{\infty} - \int (\Psi^\ast \Psi_x - \Psi \Psi^\ast_x) \, dx \right] \\
&= -\frac{i \hbar}{2m} \int (\Psi^\ast \Psi_x - \Psi \Psi^\ast_x) \, dx \\
&= -\frac{i \hbar}{2m} \left[ \int \Psi^\ast \Psi_x \, dx - \left( \Psi^\ast \Psi \Bigg \vert_{-\infty}^{\infty} - \int \Psi^\ast \Psi_x \, dx \right) \right] \\
...
&= \int \Psi^\ast (-i \hbar \frac{\partial}{\partial x}) \Psi \, dx
\end{align*}$$
