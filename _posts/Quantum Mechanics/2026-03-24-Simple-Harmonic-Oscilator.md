...
## Remark
1차원 조화 진동자를 고려하자. 즉 시스템과 각 물리량은 다음과 같이 주어진다.
...
$$\begin{align*}
\hat{a_+} &= \frac{1}{\sqrt{2 \hbar m \omega}} \left( m \omega \hat{x} - i \hat{p} \right) \\
\hat{a_-} &= \frac{1}{\sqrt{2 \hbar m \omega}} \left( m \omega \hat{x} + i \hat{p} \right)
\end{align*}$$
...
**(ii)** $\hat{H} = \hbar \omega (\hat{a_+} \hat{a_-} + 1/2) = \hbar \omega (\hat{a_-} \hat{a_+} - 1/2)$