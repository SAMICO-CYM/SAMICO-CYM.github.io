...
그러면 $(\xi, \eta)$ 좌표계에서 주어진 영역 $\Delta$의 범위를 손쉽게 표현할 수 있다. 그림에서 보이듯, 각각의 화살표는 어떤 $\xi$ 값을 고정한뒤 $\eta_0$에서부터 $\xi$까지 $\eta$에 대해 적분하는 행위를 나타낸다. 그리고 $\xi$를 $\eta_0$에서부터 $\xi_0$까지 적분하면 주어진 삼각형 영역을 표현할 수 있는 것이다. 