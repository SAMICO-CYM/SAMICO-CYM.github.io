...
그러면 Corollary (ii)의 관점에서 볼 때, $a^r$이 $G$의 생성원이라면 $n$과 $r$은 서로소여야 한다. 그리고 이를 만족하는 $r$은 정확히 $\phi(n)$개 존재하므로, $G$의 생성원은 총 [$\phi(n)$](<{% post_url Number Theory/2026-03-20-Euler-Phi-Function %}>)개 존재한다. 
