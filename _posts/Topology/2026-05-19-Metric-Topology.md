...
---
## Theorem 2
Let $d, d'$ be two metrics on a set $X, and let $\mathcal{T}_d, \mathcal{T} _ {d \prime}$ be two topologies induced by $d, d'$, respectively. Then $\mathcal{T}_{d'}$ is finer than $\mathcal{T}_d$ $\iff$ $\forall x \in X$ and $\forall \varepsilon > 0$, $\exists \delta > 0$ such that $B_{d'}(x, \delta) \subset B_d(x, \varepsilon)$.
