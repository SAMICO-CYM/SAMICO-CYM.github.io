...
**(i)** Each basis element $B$ of $\mathcal{B}$ is an element of $\mathcal{T} _{\mathcal{B}}$.

**(ii)** Every topology on a set $X$ is a basis for a topology on $X$. But the converse is **NOT** true in general.
...
$$\mathcal{T} _{\mathcal{B}} = \{ U \subset X \mid \forall x \in U, \exists B_x \in \mathcal{B} \, [x \in B_x \subset U] \}$$

is a topology on $X$, and we say that $\mathcal{T} _{\mathcal{B}}$ is ***the topology generated by $\mathcal{B}$.*** 

즉 부분집합 $U \subset X$가 기저 집합들에 대해서 open set의 성질을 만족하도록 collection을 구성하면 자연스럽게 위상 공간을 만들 수 있고, 이를 기저 $\mathcal{B}$가 생성하는 위상 공간이라고 부른다. 한편, 위의 remark에 의해서 자명하게 $\mathcal{T} = \mathcal{T} _\mathcal{T}$이 성립한다. 즉 임의의 위상에 대해서 항상 그 위상을 생성하는 기저를 찾을 수 있고, 그 기저 중 하나로 자기 자신이 주어진다.
...
Since $\\{ B_x \in \mathcal{B} \mid x \in U \\}$ is a subcollection of $\mathcal{B}$, we have $U \in T$, which means that $\mathcal{T} _{\mathcal{B}} \subset T$. Thus $T = \mathcal{T} _{\mathcal{B}}. \blacksquare$ 
...
Let $x \in B_1 \cap B_2$ for $B_1, B_2 \in \mathcal{B}$. Since $B_1, B_2 \in \mathcal{T}$, so that $B_1 \cap B_2 \in \mathcal{T}$, $\exists B_3 \in \mathcal{B}$ such that $x \in B_3 \subset B_1 \cap B_2$. Thus $\mathcal{B}$ is a basis.
